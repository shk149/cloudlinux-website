  ...
  return (
    <section ref={ref} className="section-pad bg-gradient-to-b from-[#EEF2FA] to-[#E4EAF4]">
      <div className="mx-auto max-w-[1200px]">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: 0.15 }} className="grid gap-6 lg:grid-cols-[260px_1fr]">
          {/* Products */}
          <div>
            <h3 className="mb-4 text-[12px] font-bold uppercase tracking-wider text-[#111827]">Select Products</h3>
            <div className="flex flex-col gap-2">
              {stackProducts.map((p) => (
                <label key={p.name} className={`flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-3 transition-all duration-200 ${selected.includes(p.name) ? "border-[#2563EB]/30 bg-white shadow-sm" : "border-gray-200/70 bg-white/50 hover:bg-white/80"}`}>
          ...
          {/* Features */}
          <div className="rounded-[14px] border border-gray-200/80 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)] sm:p-8">
            <h3 className="mb-6 text-[12px] font-bold uppercase tracking-wider text-[#111827]">What You Gain</h3>
            <div className="grid gap-6 sm:grid-cols-2">
              {featureCategories.map((cat) => (
                <div key={cat.title}>
                  <h4 className="mb-3 text-[14px] font-bold text-[#2563EB]">{cat.title}</h4>
