      ...
      <div className="mx-auto max-w-[1200px]">
        <div className="grid items-start gap-10 lg:grid-cols-[1fr_2fr]">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}>
            ...
            <p className="mt-3 mb-8 text-[15px] text-gray-400">Latest from our blog</p>
            <div className="hidden lg:block">
              <Image src="/illustrations/image-9.svg" alt="" width={220} height={160} className="w-full max-w-[220px] opacity-60" />
            </div>
          </motion.div>
          ...
          <div className="flex flex-col gap-4">
            {posts.map((post, i) => (
              <motion.article
                ...
                className="group flex cursor-pointer gap-4 rounded-[14px] border border-white/8 bg-white/[0.04] p-5 transition-all duration-200 hover:border-white/15 hover:bg-white/[0.07]"
              >
                ...
                <div className="min-w-0">
                  <h3 className="mb-1.5 text-[15px] font-bold leading-snug text-white transition-colors duration-200 group-hover:text-[#60A5FA]">{post.title}</h3>
